...
def get_north_end(r,c, rocks, walls):
    for nr in range(r-1, -1, -1):
        if (nr,c) in rocks or walls[nr][c]: return nr+1
    return 0
def get_east_end(r,c,rocks,walls, size):
    for nc in range(c+1, size):
        if (r,nc) in rocks or walls[r][nc]: return nc-1
    return size-1
...
def check_if_stuck(rock_history, margin):
    return False
    if len(rock_history) < margin: return False
    init_coord = rock_history[-margin]
    for coords in rock_history[-(margin-1):]:
        if coords != init_coord: return False
    return True

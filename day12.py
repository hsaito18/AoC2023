...
def check_possible(arr, groups):
    ...
    for c in arr:
        ...
        if counting:
            actual.append(curr_count)
            counting = False
            if confirmed_first and curr_count != groups[0]:
                return False
            curr_count = 0
            confirmed_first = False
    if counting:
...
def recursive_search(start, groups_text):
    ...
    for i, c in enumerate(start):
        ...
        if c == ".":
            continue
        if i + first_grouping > len(start):
            break
        if "." in start[i : i + first_grouping]:
            continue
        if i + first_grouping < len(start) and start[i + first_grouping] == "#":
            continue
        next.append(
            (start[i + first_grouping + 1 :], ",".join(str(x) for x in groups[1:]), i)
        )
    out = 0
    for poss in next:
...
def fold_line(line, folds, groups):
    return (
        (line + "?") * (folds - 1) + line + " " + (groups + ",") * (folds - 1) + groups
    )

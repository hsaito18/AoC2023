...
# Load input file:
path = os.getcwd()
with open(os.path.join(path, "inputs", "day05.txt")) as f:
    lines = f.readlines()
...
def get_mapped_inputs(lines):
    ...
    for line in lines:
        ...
        inputs.append(
            {
                "start": in_start,
                "end": in_start + range_num,
                "offset": out_start - in_start,
                "min_out": out_start,
                "max_out": out_start + range_num,
            }
        )
        if in_start < min_in:
            min_in = in_start
    if min_in > 0:
        inputs.append(
            {"start": 0, "end": min_in, "offset": 0, "min_out": 0, "max_out": min_in}
        )
    return inputs
...
def get_output(input, map):
    for map_obj in map:
        if input < map_obj["start"]:
            continue
        if input >= map_obj["end"]:
            continue
...
def get_preimage(out_start, out_end, map):
    preimage = []
    for map_obj in map:
        if out_start >= map_obj["max_out"]:
            continue
        if out_end < map_obj["min_out"]:
            continue
        ...
        preimage.append(
            {
                "start": min_pre - offset,
                "end": max_pre - offset,
                "in_start": map_obj["start"],
            }
        )
    ...
    for map_obj in map:
        for ex in extras:
            new_out_start = -1
            new_out_end = -1
            if ex["start"] >= map_obj["end"]:
                continue
            if ex["end"] < map_obj["start"]:
                continue
            ...
            elif ex["start"] >= map_obj["start"]:
                extras.remove(ex)
            else:
                ex["end"] = new_out_end
                extras.append(
                    {"start": new_out_start, "end": ex["end"], "in_start": -2}
                )
    preimage += extras
    return preimage
...
seed_to_soil_map = get_mapped_inputs(
    lines[seed_to_soil_range[0] : seed_to_soil_range[1]]
)
soil_to_fert_map = get_mapped_inputs(
    lines[soil_to_fert_range[0] : soil_to_fert_range[1]]
)
fert_to_water_map = get_mapped_inputs(
    lines[fert_to_water_range[0] : fert_to_water_range[1]]
)
water_to_light_map = get_mapped_inputs(
    lines[water_to_light_range[0] : water_to_light_range[1]]
)
light_to_temp_map = get_mapped_inputs(
    lines[light_to_temp_range[0] : light_to_temp_range[1]]
)
temp_to_humid_map = get_mapped_inputs(
    lines[temp_to_humid_range[0] : temp_to_humid_range[1]]
)
humid_to_loc_map = get_mapped_inputs(
    lines[humid_to_loc_range[0] : humid_to_loc_range[1]]
)

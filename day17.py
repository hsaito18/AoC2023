...
def get_min_heat_loss(r, c, dir):
    ...
    for direction in DIRECTIONS:
        if dir == direction or direction == get_opp_dir(dir):
            continue
        cum_heat = 0
        for dist in range(1, max_dist + 1):
            ...
            if next_heat > states.get((nr, nc, direction), float("inf")):
                continue
    ...
    visited.add((r, c, dir))
    if (r, c, dir) in unvisited:
        unvisited.remove((r, c, dir))
    min_heat = float("inf")
    next_node = -1
    # print(len(unvisited))
